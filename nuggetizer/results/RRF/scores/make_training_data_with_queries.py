#!/usr/bin/env python3
"""
Enhanced script to create training data from RRF scores files with query texts.

This script:
1. Excludes queries that exist in the test qrels file
2. Extracts version numbers from filenames
3. Loads main queries and version-specific query texts
4. Creates training data with qid, main query, version queries, versions, and performance scores
"""
...
def load_main_queries(main_topics_path):
    """Load main queries from the main topics file."""
    main_queries = {}
    with open(main_topics_path, 'r', encoding='utf-8') as f:
        for line in f:
            if line.strip():
                parts = line.strip().split('\t', 1)
                if len(parts) == 2:
                    qid, query = parts
                    main_queries[qid] = query
    return main_queries

def load_version_queries(queries_dir):
    """Load version-specific queries from all version files."""
    version_queries = {}
    
    # Find all version files
    version_files = list(Path(queries_dir).glob("topics.rag24.test.v*.txt"))
    
    for file_path in version_files:
        # Extract version number from filename
        match = re.search(r'v(\d+)\.txt', file_path.name)
        if match:
            version = int(match.group(1))
            
            # Load queries for this version
            version_queries[version] = {}
            with open(file_path, 'r', encoding='utf-8') as f:
                for line in f:
                    if line.strip():
                        ...
                        if len(parts) == 2:
                            qid, query = parts
                            version_queries[version][qid] = query
...
def process_scores_file(file_path, test_qids, main_queries, version_queries):
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            if line.strip():
                ...
                # Get main query
                main_query = main_queries.get(qid, "")
                
                # Get version-specific queries
                version_queries_text = []
                for version in versions:
                    version_query = version_queries.get(version, {}).get(qid, "")
                    version_queries_text.append(version_query)
                ...
                training_data.append({
                    ...
                    'version_queries': version_queries_text,
                    'versions': versions,
...
def main():
    ...
    main_topics_path = Path("/future/u/negara/home/set_based_QPP/data/topics.rag24.test.txt")
    queries_dir = Path("/future/u/negara/home/set_based_QPP/data/query_reformulation/queries")
    output_path = scores_dir / "training_data_with_queries.csv"
    
    print(f"Processing scores from: {scores_dir}")
    print(f"Excluding QIDs from: {test_qrels_path}")
    print(f"Loading main queries from: {main_topics_path}")
    print(f"Loading version queries from: {queries_dir}")
    ...
    # Load main queries
    main_queries = load_main_queries(main_topics_path)
    print(f"Loaded {len(main_queries)} main queries")
    
    # Load version queries
    version_queries = load_version_queries(queries_dir)
    print(f"Loaded queries for {len(version_queries)} versions")
    ...
    for file_path in scores_files:
        ...
        training_data = process_scores_file(file_path, test_qids, main_queries, version_queries)
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = [
            'qid', 
            'main_query',
            'version_0_query', 'version_1_query', 'version_2_query', 'version_3_query', 'version_4_query',
        ...
        for data in all_training_data:
            row = {
                ...
                'version_0_query': data['version_queries'][0] if len(data['version_queries']) > 0 else "",
                'version_1_query': data['version_queries'][1] if len(data['version_queries']) > 1 else "",
                'version_2_query': data['version_queries'][2] if len(data['version_queries']) > 2 else "",
                'version_3_query': data['version_queries'][3] if len(data['version_queries']) > 3 else "",
                'version_4_query': data['version_queries'][4] if len(data['version_queries']) > 4 else "",
    ...
    print(f"Training data with queries written to: {output_path}")
    ...
    for i, data in enumerate(all_training_data[:3]):
        print(f"  {i+1}. QID: {data['qid']}")
        print(f"     Main Query: {data['main_query'][:80]}...")
        print(f"     Versions: {data['versions']}")
        print(f"     Version Queries: {[q[:40] + '...' if len(q) > 40 else q for q in data['version_queries']]}")
        print(f"     Performance: {data['performance']:.4f}")
        print()
